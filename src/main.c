...
int main(int argc, char* argv[])
{
    ...
    //Lancement SDL
    SDL_initialisation();
    window = SDL_CreateWindow("[EN COURS] DE REP@RATION", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, height, width, SDL_WINDOW_FULLSCREEN_DESKTOP);
            ...
            case SDL_MOUSEBUTTONDOWN: // Relâchement d'un clique
                ...
                if ((x > rectangle_restart.x && x<(rectangle_restart.x + rectangle_restart.w) && y>rectangle_restart.y && y < (rectangle_restart.y + rectangle_restart.h)) && program_launched)
                {
                   SDL_RenderClear(renderer);
                    ...
                    if(1){  //on met à null les coordonnées des boutons restart et quitter
                    ...
                    while (SDL_QUIT)
                    {
                      ...
                      /*
                        ...
                        SDL_RenderPresent(renderer);
                        menu_select(window, renderer, rect_trajan, rect_pierre);
                        SDL_RenderPresent(renderer); */
